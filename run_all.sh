...
build_all_fuzzers() {
    ...
    # ── SQLite ────────────────────────────────────────────────────────────────
    if [ ! -f "$REPO/sqlite_libfuzzer" ] || [ ! -f "$REPO/sqlite_wingfuzzer_real" ]; then
        build "Building SQLite LibFuzzer + WingFuzz binaries..."
        "$REPO/build_comparable.sh"
    else
        build "sqlite_libfuzzer + sqlite_wingfuzzer_real already present."
    fi

    if [ ! -f "$REPO/sqlite3_DDFuzzer" ]; then
        build "Building sqlite3_DDFuzzer inside Docker..."
        docker run --rm \
            ...
            bash -c "
                ...
                CC=afl-clang-fast ./configure \
                    --enable-static --disable-shared --disable-amalgamation
                make -j\$(nproc)
                afl-clang-fast -g -O2 \
                    ...
                    -o /workspaces/CyberSecurity/sqlite3_DDFuzzer \
                    -ldl -lpthread -lm
            "
        build "sqlite3_DDFuzzer built."
    else
        build "sqlite3_DDFuzzer already present."
    fi

    # ── cJSON ─────────────────────────────────────────────────────────────────
    if [ ! -f "$REPO/cjson_libfuzzer" ] || [ ! -f "$REPO/cjson_wingfuzz_real" ] || [ ! -f "$REPO/cjson_DDFuzzer" ]; then
    ...
    # ── re2 ───────────────────────────────────────────────────────────────────
    if [ ! -f "$REPO/re2_libfuzzer" ] || [ ! -f "$REPO/re2_wingfuzz_real" ] || [ ! -f "$REPO/re2_DDFuzzer" ]; then
    ...
    # ── RHash ─────────────────────────────────────────────────────────────────
    if [ ! -f "$REPO/rhash_libfuzzer" ] || [ ! -f "$REPO/rhash_wingfuzz_real" ] || [ ! -f "$REPO/rhash_DDFuzzer" ]; then
