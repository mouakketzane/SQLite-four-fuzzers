...
check_bins() {
    ...
    if [ ! -f "$REPO/rhash_DDFuzzer" ]; then
        echo "WARNING: rhash_DDFuzzer not found — DDFuzz step will be skipped"
        HAS_DDFUZZ=0
    fi
...
if [ -x "$BUGS_SCRIPT" ]; then
    LF_BUGS=$("$BUGS_SCRIPT" "$OUTDIR/lf_corpus"  "${LF_START:-0}"  "$LF_BIN" "rhash" 2>/dev/null || echo "none")
    WF_BUGS=$("$BUGS_SCRIPT" "$OUTDIR/wf_crashes"  "${WF_START:-0}"  "$WF_BIN" "rhash" "$WF_LOG" 2>/dev/null || echo "none")
    if [ "$HAS_DDFUZZ" -eq 1 ] && [ -d "$DDF_CRASH_DIR" ]; then
        DDF_BUGS=$("$BUGS_SCRIPT" "$DDF_CRASH_DIR" "${DDF_START:-0}" "$LF_BIN" "rhash" 2>/dev/null || echo "none")
    fi
fi
